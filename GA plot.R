...
plotbars(arithmetic, singlepoint, blend)

#optimal_returns = portfolio_returns(sol)
par(mar=c(5,4,4,8),xpd=TRUE)
plot(cumsum(optimal_returns),type="l",lwd=5)
lines(cumsum(asset_returns_train[,1]),col="blue")
lines(cumsum(asset_returns_train[,2]),col="red")
lines(cumsum(asset_returns_train[,3]),col="green")
lines(cumsum(asset_returns_train[,4]),col="violet")
lines(cumsum(asset_returns_train[,5]),col="peru")
lines(cumsum(asset_returns_train[,6]),col="pink")
lines(cumsum(asset_returns_train[,7]),col="brown")
lines(cumsum(asset_returns_train[,8]),col="orange")
lines(cumsum(asset_returns_train[,9]),col="yellow")
lines(cumsum(asset_returns_train[,10]),col="grey")
lines(cumsum(asset_returns_train[,9]),col="maroon")
lines(cumsum(asset_returns_train[,10]),col="tan")
legend("left",legend=c("Weighted portfolio",names(asset_returns_train)),
       col =
         c("black","blue","red","green","violet","peru","pink","brown","orange","yellow","grey","maroon","tan"),lty=1, cex = 0.3)
